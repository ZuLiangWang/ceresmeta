...
type Server struct {
	...
	// procedureScheduler scheduler procedure for all cluster.
	procedureScheduler *coordinator.Scheduler
...
func (srv *Server) startServer(_ context.Context) error {
	...
	procedureFactory := procedure.NewFactory(id.NewAllocatorImpl(srv.etcdCli, "procedure", 5), dispatch)
	srv.procedureFactory = procedureFactory
	srv.procedureScheduler = coordinator.NewScheduler(manager, procedureManager, procedureFactory, dispatch)
...
func (c *leadershipEventCallbacks) AfterElected(ctx context.Context) {
	...
	if err := c.srv.procedureScheduler.Start(ctx); err != nil {
...
func (c *leadershipEventCallbacks) BeforeTransfer(ctx context.Context) {
	...
	if err := c.srv.procedureScheduler.Stop(ctx); err != nil {
